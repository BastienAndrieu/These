...
\begin{tikzpicture}[
	...
	% W-space
	\begin{scope}[
		shift={(-0.2,0.2)}, %shift={(-0.4,0.3)}, 
		x={0.5*\wsize}, y={0.5*\wsize}]
		\draw[axe] (-1,0) -- (1.3,0) node[label, anchor=west] {$w$};
		\draw[curv] (-1,0) -- (1,0);
		\DTLassign{dbpointoncurv}{1}{\wxco=x,\wyco=y}% 
		\coordinate (w) at (\wxco,0);
		\node[label, anchor=south, inner sep=3pt] at (w) {$w$};
		\fill[black] (w) circle (1.5pt);
		%
		%\draw [red] (0,0) circle (2.0pt);
	\end{scope}
	%
	%
	% XYZ-space
	\begin{scope}[shift={(0.2,0)}, 
		...
		{\transparent{\parampatchshadowtransparency}%
			\node[img] at (0,0) {\includegraphics[width=\parampatchimagewidth]{figures/parametric_patch_shadow}};
		}%
		\node[img] at (0,0) {\includegraphics[width=\parampatchimagewidth]{figures/parametric_patch_surface}};
		{\transparent{0.05}%
			\node[img] at (0,0) 	{\includegraphics[width=\parampatchimagewidth]{figures/parametric_patch_checkerboard6}};
		}%
		\node[img] at (0,0) {\includegraphics[width=\parampatchimagewidth]{figures/parametric_patch_border}};
	%	{\transparent{0.5}%
	%		\node[img] at (0,0) {\includegraphics[width=\parampatchimagewidth]{figures/parametric_patch_isou}};
	%		\node[img] at (0,0) {\includegraphics[width=\parampatchimagewidth]{figures/parametric_patch_isov}};
	%	}%
		% vecteurs
		\def\scalevectors{1}
	...
	\begin{scope}[shift={(0,0.75)}, x={0.5*\uvsize}, y={0.5*\uvsize}]
		% UV-domain
...
%		(current bounding box.north east);
	...
	\draw [map] (a) 
		to [bend right=20] 
		node [label, anchor=north west] {$\bp$} 
		(b);
%	\draw [map, shorten >= 15mm, shorten <= 12mm] (uv) 
%		to [bend right=30] 
%		node [label, anchor=north east, inner sep=5pt] {$\bs$} 
%		(s);
	\draw [draw=none] (uv) 
		to [bend left=30] 
		coordinate[pos=0.2] (c)
		coordinate[pos=0.5] (d)
		(s);
	\draw [map] (c) 
		to [bend left=20] 
		...
		(d);
	%
%	\circlearcparameters{(w)}{(uv)}{70}{\aangle}{\bangle}{\abradius}
%	\newlength{\arcradius}
%	\setlength{\arcradius}{\abradius}
%	\draw[red, thick, map] 
%		(w) ++ (15:\arcradius) arc ({\aangle+15}:{\bangle-15}:\arcradius) 
%		node[pos=0.5] {\the\arcradius};
\end{tikzpicture}
%\par\hrule
