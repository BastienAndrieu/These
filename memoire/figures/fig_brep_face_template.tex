...
%
\DTLloaddb[noheader,keys={id,x,y}]{dbfacexyzlabel}{figures/data/fig_brep_faces/face_xyzlabel_#1.dat}%
...
\begin{scope}[shift={({#2},{#3})}]
	...
	\begin{scope}[shift={(0.5,-0.7)}, scale={\uvscale}]
		...
		\begin{scope}[every path/.style={draw=none,fill=facecolor}]
			\path 
				plot file {figures/data/fig_brep_faces/contour_ext_#1.dat} -- cycle
				\foreach \iwint in {1,...,\numberofwires}{
					plot file {figures/data/fig_brep_faces/contour_int_#1_\iwint.dat} -- cycle
				};
		\end{scope}
		%% DRAW EDGES
