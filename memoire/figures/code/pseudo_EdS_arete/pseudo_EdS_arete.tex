...
%		\draw[vector] (e) -- (en) node[txt, left, inner sep=1pt] {$\unv_{\envelope}(u,v)$};
		...
		\node[txt, above left, inner sep=0, colorPseudoEdS!90!black] at (occ) {$\vit{o}(u)$};
		...
		\coordinate (t) at (\tx, \ty);
		\draw[vector] (g) -- (t) node[txt, below right, inner sep=0, shift={(-2pt,-1pt)}] {$\vrm{t}(u)$};
		%
		\DTLassign{dbisoupoints}{8}{\rx=x, \ry=y}%
		\coordinate (r) at (\rx, \ry);
		\draw[vector, colorPseudoEdS] (e) -- (r) node[txt, right, inner sep=1pt] {$\vrm{r}(u,v)$};
		%
		\DTLassign{dbisoupoints}{9}{\rRx=x, \rRy=y}%
		\coordinate (rR) at (\rRx, \rRy);
		\draw[vector, colorRight] (eR) -- (rR) node[txt, right, inner sep=1pt] {$\Right{\vrm{r}}(u)$};
		%
		\DTLassign{dbisoupoints}{10}{\rLx=x, \rLy=y}%
		\coordinate (rL) at (\rLx, \rLy);
		\draw[vector, colorLeft] (eL) -- (rL) node[txt, right] {$\Left{\vrm{r}}(u)$};
		%
