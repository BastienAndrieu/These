...
if False:
    ...
    freestyle = bpy.context.scene.render.layers.active.freestyle_settings
...
for ob in scene.objects:
    ob.select = False
obj = bpy.data.objects["enve_edge_8"]
obj.select = True
bpy.context.scene.objects.active = obj
obj.layers[0] = True
obj.layers[1] = False
...
obj = bpy.data.objects["enve_edge_8.001"]
bpy.context.scene.objects.active = obj
bpy.ops.object.material_slot_remove()
...
for ob in scene.objects:
    ob.select = False
obj.select = True
bpy.ops.group.create(name="group_isouv")

mesh = bpy.data.meshes["enve_edge_8.001"]
...
if scene.render.use_freestyle == True:
    bpy.ops.scene.freestyle_lineset_add()
    nls = len(freestyle.linesets)
    lineset = freestyle.linesets["LineSet "+str(nls)]
    nls = len(bpy.data.linestyles)
    linestyle = bpy.data.linestyles["LineStyle."+format(nls-1,'03')]
else:
...
lineset.group = bpy.data.groups["group_isouv"]
