...
#################################################
bpy.ops.import_scene.obj(filepath=pthin+'mesh'+suffixe+'/mesh'+suffixe+'.obj',
                         axis_forward='Y', axis_up='Z')
obj = bpy.data.objects['mesh'+suffixe]
...
"""
imsize = 800
bpy.ops.image.new(name="checker",
                  width=imsize,
                  height=imsize,
                  alpha=True,
                  generated_type='COLOR_GRID')
imgchecker = bpy.data.images["checker"]
"""
...
for iface in range(nf):
    ...
    xyz = numpy.loadtxt(pthin + 'brepmesh'+suffixe+'/xyz_' + strf + '.dat', dtype=float)
