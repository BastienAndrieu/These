\chapter[Mise en \oe uvre numérique]{Mise en \oe uvre numérique et validation de l'algorithme}
...
\subsubsection{Paramétrisation approchée de l'EdS propre d'un carreau paramétrique}
\label{section:paramétrisation_EdS_propre_carreau}
...
\subsubsection{Paramétrisation approchée de l'EdS propre d'une arête \brep\ convexe}%d'un arc de courbe}
...
On choisit une paramétrisation $\eos$ de la forme [ref vers équation paramétrisation de surface canal], \ie les courbes iso-$u$ sont des arcs de cercles caractéristiques dont les extrémités $\eosR$ et $\eosL$ sont respectivement atteintes en $v = \lo{v}$ et $v = \hi{v}$. 
...
\subsubsection{Paramétrisation approchée de l'EdS propre d'un sommet \brep\ convexe}
\begin{enumerate}
	\item polygone sphérique découpé en quadrilatères (valable seulement si la vitesse normale est uniforme)
	\begin{itemize}
		\item[$-$] valable seulement si la vitesse normale est uniforme
		\item[$-$] génère plusieurs nouveaux carreaux de surface
		\item[$+$] les nouveaux carreaux sont entiers (domaine paramétrique égal à $\chebinterval \times \chebinterval$) $\Rightarrow$ permet l'utilisation de quadrature simple (\cf \autoref{section:clenshaw_curtis_quadrature})
	\end{itemize}
	\item ajustement de carreau sphérique à un nuage de points (\cf notes Huygens)
	\begin{itemize}
