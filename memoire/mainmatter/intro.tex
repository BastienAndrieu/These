...
\begin{figure}
\centering
\input{figures/singular_point_formation.tikz}
...
\begin{figure}
\centering
\input{figures/brep_hierarchy.tikz}
...
\begin{figure}
	...
	\begin{tikzpicture}[x=\imw, y=\imh, 
		...
		%%% FACES
		\node[img] at (0,0) {\includegraphics[width=\imw]{figures/fig_brep}};
		%%% HIDDEN EDGES
		\node[img] at (0,0) {\includegraphics[width=\imw]{figures/fig_brep_edges_hid}};%
		%%% HIDDEN VERTICES
		...
		%%% VISIBLE EDGES
		\node[img] at (0,0) {\includegraphics[width=\imw]{figures/fig_brep_edges_vis}};%
		%%% VISIBLE VERTICES
		\DTLforeach*{dbverts}{\locx=x, \locy=y, \loca=a}{%
			\ifnum \loca = 1
				\fill[black] (\locx,\locy) circle (1.0pt);
...
%\hrule\par
\centering
\input{figures/simple_intersection.tikz}
%\par\hrule
\caption{Description géométrique d'une arête \brep.}
\end{figure}
...
\begin{tikzpicture}
	\begin{axis}[
		hide axis,
		xmin=-2.36882376671 , xmax=2.75445795059,
		ymin=-2.34076094627 , ymax=2.61758756638,
		zmin=-0.358607441187 , zmax=1.32676851749,]
    	\addplot3 graphics [
			points={
			(0,0,0) => ( 402.2570610046387, 255.00264167785645 )
			(1,0,0) => ( 496.9456672668457, 202.3828148841858 )
			(0,1,0) => ( 495.6686019897461, 309.8987698554993 )
			(0,0,1) => ( 400.9244918823242, 364.14011716842657 )
			}] {figures/fig_simple_intersection_ortho};
		\addplot3[black, thick] table {/d/bandrieu/GitHub/FFTsurf/test/demo_intersection/simple/curve_xyz.dat};
	\end{axis}
\end{tikzpicture}
\end{figure}
