...
%	legend style={
%				font=\small,
%				/tikz/every even column/.append style={column sep=8mm}
%                },
%	legend entries={$\; k=0$,$ \; k=1$, $\; k=2$}]
%	\addlegendimage{only marks, mark=square*,   mark size=1.2pt, s1}
%	\addlegendimage{only marks, mark=*,         mark size=1.5pt, s2}
%	\addlegendimage{only marks, mark=triangle*, mark size=1.7pt, s3}
%\end{axis}
%%\node (k0)  {\protect\legendsquare{s1}\ $k = 0$};
%%\node[right of=k0] (k1) {\protect\legenddot{s2}\ $k = 1$};
%%\node[right of=k1] (k2) {\protect\legendtriangle{s3}\ $k = 2$};
%\end{tikzpicture}
%
...
\begin{semilogyaxis}[
	...
	%\addlegendentry{$1.8^{-N}$}%{$e^{-0.57N}$}
	\pgfplotsinvokeforeach{1,...,3}{
		\addplot+ table[x index=0, y index=#1] {figures/data/convergence_DNkf.dat};%
	}
%	\node at (axis cs:5,2.24e-2) {\protect\legendsquare{s1}};
%	\node at (axis cs:5,1.05e-1) {\protect\legenddot{s2}};
%	\node at (axis cs:5,9.1e-1) {\protect\legendtriangle{s3}};
...
%	\addplot+[only marks] table[x expr=\coordindex, y index=0] {figures/data/coeffs2.dat};%
%	\addplot+[only marks] table[x expr=\coordindex, y index=1] {figures/data/coeffs2.dat};%
%	\addplot+[only marks] table[x expr=\coordindex, y index=2] {figures/data/coeffs2.dat};%
	%\addplot+[dashed, color=black, no marks, very thick][domain=30:50.5] 	 {1e6*exp(-0.53*x)};%
	%\legend{{$f$},{$f'$},{$f''$},{$e^{-0.5n}$}}%
	%\legend{{$k=0$},{$k=1$},{$k=2$}}%,{$e^{-0.53n}$}}%
\end{semilogyaxis}%
...
\hspace*{\fill}%
%\caption{Précision spectrale \ldots $f : x \mapsto e^{\sin 3 x^3}$. $k = 0 (\protect\legendsquare{s1}), 1 (\protect\legenddot{s2}), 2 (\protect\legendtriangle{s3})$.}%
