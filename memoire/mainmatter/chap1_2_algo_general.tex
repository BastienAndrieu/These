...
%		où $v_{\brepedge_i} = 
%		\begin{cases}
%			\hi{v} \text{ si } u_{\brepedge_i} = \lo{u}, \\ 
%			\lo{v} \text{ si } u_{\brepedge_i} = \hi{u}.
%		\end{cases}$
...
\begin{enumerate}
	...
	\end{align} 
	 ...
	 \item on note $\unv_i$ la direction normale à $\EdSpropreplus{\brepface_i}$ au point $\eos^{\brepface_i}(u_{\brepface_i}, v_{\brepface_i})$
	 \item on choisit $\bb_1 = \unitized{\sum_{i=0}^{m-1} \unv_i}$
    \item on note $\p_i = \frac{\p}{\dot_prod{\unv_i}{\bb_1}}$ la projection centrale de $\unv_i$ sur le plan tangent à la sphère unité au point $\bb_1}$
