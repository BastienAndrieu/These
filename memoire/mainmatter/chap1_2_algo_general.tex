...
définitions : 
\begin{itemize}
%	\item \textit{zone d'influence}\footnote{\label{note_vocabulaire_notation}vocabulaire et notation temporaires \ldots} de $\region \subseteq \interface$ sur l'EdB  (lieu des points de tangence entre $\EdB{\interface}{\rho}$ et $\EdS{\region}{\rho}$)%$\sphere[\region][\rho]$)
%	\[ \influEdB{\region}{\rho} \defini{=} \EdB{\interface}{\rho} \cap  \EdS{\region}{\rho} \]
	\item \textit{EdS propre}\footref{note_vocabulaire_notation} de $\region \subseteq \interface$ 
	\[ \EdSpropre{\region}{\rho} \defini{=} \closure{ {\EdS{\region}{\rho} \setminus \EdS{\boundary{\region}}{\rho}} } \]
\end{itemize}
arguments :
\begin{enumerate}
...
%        Or, par définition, $\influEdB{\region}{\rho} \subseteq \EdS{\region}{\rho}$, donc finalement 
%        \[
...
%    \end{proof}
   ...
   % \item MIEUX : Si $\dim \region < 2$ alors $\EdB{\interface}{\rho} \cap \EdS{\region}{\rho} \setminus \pseudoEdS{\region}{\rho} = \varnothing$ ($\Leftrightarrow \EdB{\interface}{\rho} \cap \EdS{\region}{\rho} \subseteq \pseudoEdS{\region}{\rho}$)
    \item Si $\dim \region < 2$ alors $\EdB{\interface}{\rho} \cap \EdS{\region}{\rho} \subseteq \bigcap_{\regionB \in \adherentsDimSup{\region}} \EdS{\regionB}{\rho}$)
    \begin{proof}
    	Soient $\region \subset \interface$ ($\dim\region < 2$), $\regionB \in \adherentsDimSup{\region}$ et $\bx \in \EdB{\interface}{\rho} \cap \EdS{\region}{\rho}$.
    	\par
    	D'après le lemme \ref{observ:EdS_implicit_negative}, on a $\implicitEdB{\region}(\bx) \leq 0$ et, par définition, $\implicitEdB{\interface}(\bx) = 0$.
    	\par
    	Or, puisque $\region \subset \interface$, on a également $\implicitEdB{\region} \geq \implicitEdB{\interface}$, donc $\implicitEdB{\region}(\bx) = 0$. 
    	De même, puisque $\regionB \subset \interface$, on a $\implicitEdB{\regionB} \geq \implicitEdB{\interface}$ et donc $\implicitEdB{\regionB}(\bx) \geq 0$. 
    	Mais puisque $\region \subseteq \boundary \regionB$, on a aussi $\implicitEdB{\regionB} \leq \implicitEdB{\region}$.
    	On a alors $\implicitEdB{\regionB}(\bx) = 0$, soit $\bx \in \EdB{\regionB}{\rho} \subseteq \EdS{\regionB}{\rho}$. 
    	\par
    	Ceci étant vrai pour tout $\regionB \in \adherentsDimSup{\region}$, on a finalement $\EdB{\interface}{\rho} \cap \EdS{\region}{\rho} \subseteq \bigcap_{\regionB \in \adherentsDimSup{\region}} \EdS{\regionB}{\rho}$.
    \end{proof}
