...
\begin{enumerate}
	...
	où
    \begin{enumerate}
        \item $\mathbf{R} = \colvec{\br_1 & \br_2 & \br_3} \in \rotationgroup{3}$ désigne une matrice de rotation ($\transpose{\mathbf{R}} \mathbf{R} = \identitymatrix{}$ et $\determinant{\mathbf{R}} = 1$)% ($\dotprod{\br_i}{\br_j} = \delta_{i,j}$ et $\br_{i+2} = \crossprod{\br_i}{\br_{i+1}}$)
        \item $\vit{s} = 
		\begin{pmatrix}
            \cos \theta \cos \lambda \\
            \sin \theta \cos \lambda \\
