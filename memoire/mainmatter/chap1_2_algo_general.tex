...
\begin{enumerate}
	\item les régions de l'EdS partielle qui sont exclues de l'EdB sont délimitées par les bords \guillemets{naturels} des carreaux et par les courbes d'intersections entre les carreaux (argument à introduire plus tôt\ldots)\\
	\begin{proof}
...
%	\item on complète ainsi le graphe des intersections, qui donne la topologie de l'agencement des carreaux (relations d'adjacences)
	...
	\begin{itemize}
		\item délimiter les faces (\cf \autoref{section:def_brep_faces}), \ie éliminer les régions des carreaux qui sont exclues de l'EdB (\ie tout région $G$ pour laquelle il existe une région $H$ telle que $\forall \bx \in G, \implicitEdB{H}(\bx) < 0$) vérifiant les critères donnés dans la \autoref{section:def_brep_faces} :
		\begin{enumerate}
