...
\begin{enumerate}
	\item $\brepedge$ une arête \brep\ convexe, $\bg : \left[ \lo{u}, \hi{u} \right] \to \reals^3$ une paramétrisation de son support géométrique
...
Le domaine paramétrique du carreau décrivant $\pseudoEdS{\brepedge}{\rho}$ est donc le rectangle $\mathcal{U} = \left[ \lo{u}, \hi{u} \right] \times \left[ \lo{v}, \hi{v} \right]$.
	...
	\item on a $r = \normtwo{\Right{\eos} - \bo} = \normtwo{\Left{\eos} - \bo}$ (pour l'implémentation, on choisit $r(u,v) = \left( 1 - \frac{v - \lo{v}}{\hi{v} - \lo{v}} \right) \normtwo{\Right{\eos} - \bo} + \frac{v - \lo{v}}{\hi{v} - \lo{v}} \normtwo{\Left{\eos} - \bo}$)
	...
	\begin{equation}
	    \theta = 
	    \begin{cases}
	       \vartheta + 2\pi & \text{ si } \theta' < 0, \\ 
	       \vartheta        & \text{ si } \theta' \geq 0,
	    \end{cases}
	\end{equation}
	où
	\begin{equation}
...
%	\end{equation}
	...
	\end{equation}	
	De cette façon, $\br$ vérifie bien les conditions \eqref{eq:parametrisation_EdS_propre_courbe_condition_r} et on a bien $\br(u,\lo{v}) = \Right{\br}$ et $\br(u,\hi{v}) = \Left{\br}$ (donc $\eos(u,\lo{v}) = \Right{\eos}$ et $\eos(u,\hi{v}) = \Left{\eos}$).
...
\begin{enumerate}
	...
	\item[$\Rightarrow$] $\pseudoEdS{\vit{v}}{\rho}$ est le polygone sphérique formé par les $m$ points $\p_i = \vit{v} + \rho \unv_{\Sigma_i}$ ($i = 1, \ldots, m$)	où $\Sigma_i$ est le carreau de la $i$-ème face incidente à $\vit{v}$ (le cycle des faces incidentes est parcouru dans le sens direct vu de l'extérieur de $\Omega$)
	...
	\begin{itemize}
		\item on note $\vit{c} = \vit{v} + \rho \unitized{\sum_{i=1}^m \unv_{\Sigma_i}}$ et $\q_i = \vit{v} + \rho \unitized{\unv_{\Sigma_i} + \unv_{\Sigma_{i+1}}}$
		\item on construit $m$ carreaux sphériques dont les \guillemets{coins} sont $\vit{c}$, $\q_{i-1}$, $\p_i$ et $\q_i$
	\end{itemize}
	\item dans les deux cas, les carreaux sphériques sont construits en projetant sur $\sphere[\vit{v}][\rho]$ le carreau bilinéaire qui interpole ses coins
\end{enumerate}
