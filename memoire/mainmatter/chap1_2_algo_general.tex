...
%	\item La zone d'influence sur l'EdB d'une courbe $\Gamma \subset \interface$ adhérente aux nappes régulières $\Left{\nappe}$ et $\Right{\nappe}$ est incluse dans le lieu des points de tangence entre $\EdS{\Gamma}{\rho}$, $\EdS{\Left{\nappe}}{\rho}$ et $\EdS{\Right{\nappe}}{\rho}$. On appelle ce lieu \textit{pseudo-EdS}\footref{note_vocabulaire_notation} de $\Gamma$ et on le note $\pseudoEdS{\Gamma}{\rho}$.
%	\begin{proof}
%		\cf notes ``Huygens'', section ``Observations et preuves''
%	\end{proof}
%	
%	\item Si $\Gamma$ est une courbe singulière concave de $\interface$ alors sa zone d'influence sur l'EdB est vide (\ie $\Gamma$ ne contribue pas à l'EdB et peut donc être ignorée).
%	\begin{proof}
%		On veut montrer que $\influEdB{\Gamma}{\rho} = \varnothing$ (piste : montrer que si $\Gamma$ est concave alors, pour $\bx \in \complement{\solide}$, $\implicitEdB{\Gamma}(\bx) = 0 \Rightarrow \implicitEdB{\Left{\nappe}}(\bx) < 0$ ou $\implicitEdB{\Right{\nappe}}(\bx) < 0$) [\ldots]
%	\end{proof}
%	
...
%	\end{itemize}
%	On appelle cette réunion l'\textit{EdS partielle}\footref{note_vocabulaire_notation}.
...
%%On note $G' := \properEoS{G}{\rho} = \levelset_G^{-1}(0)$, $H' := \properEoS{H}{\rho} = \levelset_H^{-1}(0)$ et $\Gamma := G' \cap H'$.
	\end{proof}
	...
	%\item $\Rightarrow$ On construit d'abord l'EdS partielle puis on en élimine les régions qui sont exclues de l'EdB (au passage on constitue les relations topologiques qui assurent la validité du modèle \brep)
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item on construit $m$ carreaux sphériques $\left\{ \pseudoEdS{\brepvertex}{\rho}[i] \right\}_{i = 1, \ldots, m-1}$ dont les \guillemets{pics} sont $\vrm{q}$, $\vrm{m}_{\imoins}$, $\unv_i$ et $\vrm{m}_i$ (voir \autoref{fig:pseudo_EdS_sommet_quadpoly} droite)
