...
%[décomposition en nappes régulières ($\contgeom{1}$), crêtes (crêtes?) et points irréguliers (pics?) (\cf notes Huygens)]
%\par\bigskip
...
%	\item \textit{pic} : point qui n'est à l'intérieur d'aucune nappe régulière ou crête
%\end{enumerate}

\begin{definition}[Nappe régulière]
...
%\begin{itemize}
%	\item la frontière d'une nappe régulière est composée de crêtes et de pics (si elle n'est pas vide)
%	\item la frontière d'une crête (ses extrémités) est composée de pics (si elle n'est pas vide)
...
%La frontière $\boundary{\solide}$ du solide $\solide$ est une surface de continuité $\contgeom{1}$ (\ie possède une direction normale continue) \piecewise. 
%Les singularités géométriques de $\boundary{\solide}$ peuvent être de dimension 1 (courbes) ou 0 (points).
%Les crêtes de $\boundary{\solide}$ sont elle-mêmes des variétés différentielles de continuité $\contgeom{1}$ (\ie possède une direction tangente continue) excepté aux pics.
...
\begin{enumerate}
	...
	\item connectivité entre les entités capturée à l'aide d'une structure de graphe (\textit{carte combinatoire})%(homéomorphe à un polyèdre) 
	\item structure de données adaptée $\to$ \anglais{Doubly Connected Edge List} (DCEL) (\anglais{Half-edges}) \textit{(donner réfs.)}
\end{enumerate}
...
\begin{itemize}
	...
	%\item l'ensemble des pics de $\interface$ est un sous-ensemble des sommets de son modèle \brep\ ;
	%\item l'ensemble des crêtes de $\interface$ est un sous-ensemble des arêtes de son modèle \brep\ ;
	%\item les nappes régulières de $\interface$ sont formées par la réunion des faces de son modèle \brep.
...
\begin{enumerate}
	...
	%\item formaliser la notion d'enveloppe des sphères $\EdS{\interface}{\rho}$ (surface tangente à toutes les sphères et en tout point tangente à une sphère) $\to$ système d'équations avec $\implicitsphere_{\rho} : (\bx,\p) \mapsto \normtwo{\bx - \p}^2 - \rho^2(\p)$
	%\item formaliser la notion d'enveloppe des boules $\EdB{\interface}{\rho} \defini{=} \boundary{\left\{\solide \cup \sphere[\interface][\rho]\right\}}$
			...
			postaction={
				...
				decoration={
					...
					mark=at position 0.55 with {
						\node at (0,-1.5ex) {$\interface$};
					};
				}
			}, 
		] \pathSigma;
		\node[black!60!white] at (-0.06,-0.055) {$\solide$};
		\node[black!30!white] at (0.45,0.45) {$\complement{\solide}$};
		%\node[above] at (-0.5,0.28) {$\interface$};
	\end{tikzpicture}
	\caption{Schéma d'une interface régulière \piecewise\ en deux dimensions.}
...
%		\fi
	...
	\caption{Schéma pour distinguer EdS (haut) et EdB (bas) (à détailler \ldots).}
	\label{fig:EdS_EdB}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
