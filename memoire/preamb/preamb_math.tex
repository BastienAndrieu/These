...
\renewcommand*{\bs}{\bm{\sigma}} % originally backslash in typewriter font
...
\newcommand*{\intervalLoHi}[1]{\left[ \lo{#1}, \hi{#1} \right]}
\newcommand*{\fullUVdomain}{\lo{\hi{\uvdomain}}}
\newcommand*{\fullWdomain}{\lo{\hi{\wdomain}}}

% Restriction
%https://tex.stackexchange.com/questions/351004/typesetting-a-function-restriction-with-a-bar-shorter-than-big
...
\DeclareMathOperator{\twin}{twin}%
\DeclareMathOperator{\suiv}{next}%
\DeclareMathOperator{\prev}{prev}%
\DeclareMathOperator{\face}{face}%
\DeclareMathOperator{\coedge}{coedge}%
...
\newcommand*{\EdBplus}[2]{{#1}_{+}^{#2}}                               % EdB
\newcommand*{\EdBmoins}[2]{{#1}_{-}^{#2}}                               % EdB
...
\newcommand*{\eos}{\bm{e}}                                    % paramétrisation de l'EdS (propre)
...
\NewDocumentCommand{\pseudoEdS}{m m o}{%
	\IfValueTF{#3}{%
		\ensuremath{\widehat{\envelope}_{#3}^{#2}({#1})}%
	}{%
		\ensuremath{\widehat{\envelope}^{#2}({#1})}%
	}%
}
%\newcommand*{\proof}[1]{\textcolor{blue!60!green}{\textbf{Preuve : }#1$\,_\blacksquare$}}

\newcommand*{\identitymatrix}[1]{\mathds{1}_{#1}}
\newcommand*{\rotationgroup}[1]{\mathrm{SO(#1)}}
...
\newcommand{\defini}[1]{\overset{\underset{\mathrm{def}}{}}{#1}}